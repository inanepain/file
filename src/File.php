 ...
 * File metadata
 *
 * @method \Inane\File\File getFileInfo()
 *
 * @version 0.17.0
 */
class File extends SplFileInfo implements FSOInterface {
    ...
    /**
     * Copies the current item to the specified destination.
     *
     * @since 0.17.0
     *
     * @param self|string $dest The destination path or an instance of the same class to which the item is copied.
     *                          - This can be a directory or complete file path.
     *
     * @return bool|File false on failure, otherwise the destination File instance.
     */
    public function copy(self|string $dest): false|self {
        $dest = is_string($dest) ? new File($dest) : $dest;
        if ($dest->isDir()) {
            $dest = $dest->getFile($this->getFilename());
        }
        if ($this->isValid() && $dest->getParent()->isWritable()) {   // * True if a file exists | Tells whether the filename is writable
            if (copy($this->getPathname(), $dest->getPathname())) {   // Copies file
                return $dest;
            }
        }
